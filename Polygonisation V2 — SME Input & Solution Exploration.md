...
1. Use the histogram to control which parts of the DEM are visualised as cut/fill regions.
2. Apply thresholds to reduce noise.
3. Visualise the resulting cut/fill regions directly on the map.
4. Define polygonisation parameters such as **minimum polygon area, layer name, etc.**
5. Generate multiple cut/fill polygon layers from a Subtract DEM based on the selected histogram ranges.
